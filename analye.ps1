...
mypy "$ProjectPath" --disallow-untyped-defs --disallow-untyped-calls --ignore-missing-imports --exclude build/ --explicit-package-bases
...
$pyFiles = Get-ChildItem -Path $ProjectPath -Recurse -Include *.py | ForEach-Object { $_.FullName }
if ($pyFiles.Count -eq 0) {
    Write-Host "No Python files found to analyze."
} else {
    python -m vulture $pyFiles --min-confidence 80 --exclude "infra/build"
}
